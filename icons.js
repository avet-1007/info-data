/* ============================================================================
 * icons.js — Inlined Lucide icons (MIT). Vendored as raw SVG path data so the
 * app stays fully self-contained / offline. https://lucide.dev
 *
 *   Icon(name, { size, cls, color }) -> SVG markup string
 *   fileIcon(path) -> { icon, color } chosen by file type (VS Code-ish colors)
 * ==========================================================================*/
...
const ICONS = {
  ...
  'terminal-panel': '<rect width="18" height="18" x="3" y="3" rx="2"/><path d="M3 15h18"/><path d="m9 10 3-3 3 3"/>',
  ...
  'cursor': '<path d="M12.034 12.681a.498.498 0 0 1 .647-.647l9 3.5a.5.5 0 0 1-.033.943l-3.444 1.068a1 1 0 0 0-.66.66l-1.067 3.443a.5.5 0 0 1-.943.033z"/><path d="M21 11V5a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h6"/>',
...
function Icon(name, opts = {}) {
  ...
  return `<svg${cls}${style} width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${inner}</svg>`;
}

// File-type icon + accent color, VS Code-ish.
const FILE_ICONS = {
  markdown:   { icon: 'file-text', color: '#9aa6ff' },
  html:       { icon: 'file-code', color: '#e06c75' },
  css:        { icon: 'file-code', color: '#61afef' },
  javascript: { icon: 'file-code', color: '#e5c07b' },
  json:       { icon: 'file-json', color: '#98c379' },
  python:     { icon: 'file-code', color: '#56b6c2' },
  shell:      { icon: 'file-code', color: '#98c379' },
  text:       { icon: 'file-text', color: '#9aa0a6' },
};

function fileIcon(path) {
  if (/\.drawio$/i.test(path)) return { icon: 'shapes', color: '#8aadf4' };
  if (/\.(glb|gltf|obj|stl|ply|fbx|3mf|dae)$/i.test(path)) return { icon: 'box', color: '#f5a97f' };
  if (/\.(png|jpe?g|gif|svg|webp|bmp|ico)$/i.test(path)) return { icon: 'image', color: '#c678dd' };
  if (/\.(mp4|webm|mov|avi|mkv|m4v)$/i.test(path)) return { icon: 'box', color: '#e06c75' };
  if (/\.(zip|rar|7z|tar|gz|tgz)$/i.test(path)) return { icon: 'package', color: '#e5c07b' };
  const lang = (typeof FileTypes !== 'undefined') ? FileTypes.detect(path) : 'text';
  return FILE_ICONS[lang] || { icon: 'file', color: '#9aa0a6' };
}

// Real file-type logos (vscode-icons, MIT) loaded from CDN as <img>, with a
// Lucide fallback (via fileIcon) when offline or for types without a logo.
const VSCODE_ICON_BASE = 'https://cdn.jsdelivr.net/gh/vscode-icons/vscode-icons@master/icons/file_type_';
const EXT_LOGO = {
  ...
  json: 'json', py: 'python', sh: 'shell', bash: 'shell', zsh: 'shell', xml: 'xml',
  txt: 'text', log: 'text', svg: 'svg', drawio: 'drawio', pdf: 'pdf',
...
function fileLogoURL(path) {
  const m = /\.([a-z0-9]+)$/i.exec(path || '');
  const key = m && EXT_LOGO[m[1].toLowerCase()];
  return key ? VSCODE_ICON_BASE + key + '.svg' : null;
}

window.Icon = Icon;
window.fileIcon = fileIcon;
window.fileLogoURL = fileLogoURL;
